...
- Handle the "Game Over" state should have higher priority than the collision handling.
- Moving enemies should be able to collide with the player to trigger the "Game Over" state.

#### Game Over state
- Display a "Game Over" overlay (a `div` with class `game-over`).
