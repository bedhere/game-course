...
Let’s set the rules:
- **survival points** will add 1 point for every 10 seconds the player stays alive.
- **kill points** will add 10 points for each enemy defeated.

Track survival time using a `Clock` and update the score, so survival points accumulate continuously during gameplay.
...
Use the specification from the `spec.md` file to implement the scoring system. It should handle time-based scoring with a `Clock`, add points for defeated enemies, and keep the score display updated on screen.
